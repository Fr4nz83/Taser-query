...
public:
	...
	virtual void regenerateGraph(const uint32_t& numPOIsCOI = 50) {};
	...
	void precomputeTDPOIsImproved(const std::string& outputFile);
	void findNNDijkstra(const uint32_t& src,
	...
	virtual void loadPrecomputedTDPOIs(const std::string& nameFile);
